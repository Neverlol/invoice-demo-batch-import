...
已接通但仍需用辽宁税局真实失败明细继续打磨的能力：

- 税局失败明细回流到草稿行。
- 对税率等有明确建议值的失败项，一键应用建议并重建模板。
- 对编码不允许、必填缺失等高频失败继续补充分类和修复提示。

## 官方模板版本

税局可能定期更新批量导入模板。当前导出器不再硬编码单一旧模板，而是会从 `official_templates/` 中自动选择文件名里 `(Vxxxxxx版)` 版本号最高的 `.xlsx` 作为导出底板。

当前已验证的新模板：

- `(V260401版)批量开票-导入开票模板.xlsx`
- 内部 `excelVersion`: `pt:20260401`

如果税局提示“导入模板已更新”，短期处理方式是：从税局下载最新空模板，放入 `official_templates/`，文件名保留 `(Vxxxxxx版)` 版本号，然后重新生成导入 Excel。后续应产品化为“上传最新官方模板并升级”的工作台功能。
...
1. 用辽宁省电子税务局真实材料做 Windows 端到端预演。
2. 用税局失败明细继续扩展本地预校验规则和失败分类。
3. 部署阿里云 sync center，验证 Windows 客户端 case event 回传。
4. 申请并配置 MiniMax M2.7 API Key，做真实抽取 smoke test。
5. 继续补机动车/建筑服务/旅客运输等强特定业务模板化。
