...
```text
未勾选批量模式：
  始终按单张发票处理；即使上传多个文件 / 多张图片，也视为同一张发票的材料。只要有图片 / PDF / Excel / 文档等附件，就默认进入 LLM / 视觉识别链路，不再因为规则看起来可用或未覆盖格式而跳过大模型。

勾选批量模式：
  才进入批量处理；图片按“一张图一张子草稿”处理，缺字段进入待补全。

批量图片数量建议：
  1-5 张：默认逐张调用视觉大模型提取核心信息，仍保持“一张图一张票”的业务边界。
  超过 5 张：回到 OCR / 文档解析 / 本地规则 / 待补全的混合模式，避免等待时间过长、模型费用上升和多图串单风险。

对客户建议口径：
  现场建议每批控制在 3-5 张以内；超过 5 张可以继续生成待复核草稿，但不承诺全部直接由大模型完整识别。
```
