...
@dataclass(frozen=True)
class InvoiceHistoryRecordMatch:
    buyer: BuyerInfo
    invoice_kind: str
    amount_with_tax: str
    note: str
...
def resolve_invoice_record_from_history(
    *,
    company_name: str,
    buyer: BuyerInfo,
    bill_id: str = "",
    amount_with_tax: str = "",
) -> InvoiceHistoryRecordMatch | None:
    seller_query = company_name.strip()
    bill_id = re.sub(r"\D", "", str(bill_id or ""))
    amount = str(amount_with_tax or "").strip()
    for seller in _load_cached_sellers():
        ...
        if seller_query and seller_query not in {seller_name, seller_tax_id}:
            continue
        for record in seller.get("invoice_records") or []:
            if not isinstance(record, dict):
                continue
            if buyer.tax_id and str(record.get("buyer_tax_id") or "").strip() != buyer.tax_id.strip():
                continue
            if not buyer.tax_id and buyer.name and str(record.get("buyer_name") or "").strip() != buyer.name.strip():
                continue
            record_note = str(record.get("note") or "")
            if bill_id and bill_id not in re.sub(r"\D", "", record_note):
                continue
            if amount and _money_key(record.get("amount_with_tax") or "") != _money_key(amount):
                continue
            lines = [_cached_invoice_line_to_model(line) for line in record.get("lines") or [] if isinstance(line, dict)]
            if not lines:
                continue
            return InvoiceHistoryRecordMatch(
                buyer=BuyerInfo(name=str(record.get("buyer_name") or "").strip(), tax_id=str(record.get("buyer_tax_id") or "").strip()),
                invoice_kind="增值税专用发票" if "专用" in str(record.get("invoice_kind") or "") else "普通发票",
                amount_with_tax=str(record.get("amount_with_tax") or "").strip(),
                note=record_note,
                lines=lines,
                invoice_no=str(record.get("invoice_no") or "").strip(),
                confidence="high" if bill_id else "medium",
            )
    return None


def _cached_invoice_line_to_model(line: dict) -> InvoiceLine:
    return InvoiceLine(
        project_name=str(line.get("project_name") or "").strip(),
        amount_with_tax=str(line.get("amount_with_tax") or "").strip(),
        tax_rate=str(line.get("tax_rate") or "").strip(),
        ...
        coding_reference="客户档案历史发票精确匹配，需人工复核",
    )
