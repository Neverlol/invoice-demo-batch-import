...
class MiniMaxOpenAICompatibleAdapter(BaseLLMAdapter):
    ...
    def extract_invoice_info(self, text: str) -> LLMResponse:
        prompt = (
            ...
            "如果是维修站给保险公司开票，PDF/发票样张中的公司通常是购买方，PDF/发票样张中的项目和税率可作为本次类目参考。\n"
            "如果补充文字里只有‘3000元’这类金额，必须把它作为本次开票金额；PDF/发票样张里的旧金额只是历史样张，不能覆盖补充文字金额。\n"
            "车辆照片或聊天记录中的车牌号应放入顶层‘备注’，不要放入项目名称。\n"
    ...
    def extract_invoice_info_from_images(self, text: str, image_paths: list[Path]) -> LLMResponse:
        ...
        prompt = (
            ...
            "如果补充文字里只有‘3000元’这类金额，必须把它作为本次开票金额；PDF/发票样张里的旧金额只是历史样张，不能覆盖补充文字金额。\n"
            "车辆照片或聊天记录中的车牌号应放入顶层‘备注’，不要放入项目名称。\n"
            "车牌号通常形如“辽A12345、辽AD12345、辽A·12345、辽A12345D”等；仅在图片/文字清晰可辨时填写，无法确认则备注返回空字符串。\n"
            "JSON 顶层字段必须包含：客户名称、纳税人识别号、地址电话、开户行及账号、项目列表、价税合计、备注；若能识别发票类型，也返回发票类型。\n"
            "补充文字如下：\n"
            f"{text or '无'}"
        )
